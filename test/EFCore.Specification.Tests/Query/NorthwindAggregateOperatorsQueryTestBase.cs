    ...
    where TFixture : NorthwindQueryFixtureBase<NoopModelCustomizer>, new()
{
    ...
    public virtual Task Sum_over_subquery(bool async)
        => AssertSum(
            async,
            ss => ss.Set<Customer>(),
            selector: c => c.Orders.Sum(int (o) => o.OrderID));
    ...
    public virtual Task Sum_over_nested_subquery(bool async)
        => AssertSum(
            async,
            ss => ss.Set<Customer>(),
            selector: c => c.Orders.Sum(int (o) => 5 + o.OrderDetails.Sum(int (od) => od.ProductID)));
    ...
    public virtual Task Sum_over_min_subquery(bool async)
        => AssertSum(
            async,
            ss => ss.Set<Customer>(),
            selector: c => c.Orders.Sum(int (o) => 5 + o.OrderDetails.Min(int (od) => od.ProductID)));
    ...
    public virtual Task Sum_on_float_column_in_subquery(bool async)
        => AssertQuery(
            async,
            ss => ss.Set<Order>().Where(o => o.OrderID < 10300).Select(o => new { o.OrderID, Sum = o.OrderDetails.Sum(od => od.Discount) }),
            e => e.OrderID);
    ...
    public virtual Task Average_on_float_column_in_subquery(bool async)
        => AssertQuery(
            ...
            ss => ss.Set<Order>().Where(o => o.OrderID < 10300)
                .Select(o => new { o.OrderID, Sum = o.OrderDetails.Average(od => od.Discount) }),
            e => e.OrderID);
    ...
    public virtual Task Min_no_data(bool async)
        => Assert.ThrowsAsync<InvalidOperationException>(() => AssertMin(
            async,
    ...
    public virtual Task Average_no_data(bool async)
        => Assert.ThrowsAsync<InvalidOperationException>(() => AssertAverage(
            async,
            ss => ss.Set<Order>().Where(o => o.OrderID == -1),
            selector: o => o.OrderID));
    ...
    public virtual Task Min_over_nested_subquery(bool async)
        => AssertMin(
            async,
            ss => ss.Set<Customer>().OrderBy(c => c.CustomerID).Take(3),
            selector: c => c.Orders.Min(o => 5 + o.OrderDetails.Min(int (od) => od.ProductID)));
    ...
    public virtual Task Max_over_nested_subquery(bool async)
        => AssertMax(
            async,
            ss => ss.Set<Customer>().OrderBy(c => c.CustomerID).Take(3),
            selector: c => c.Orders.Max(o => 5 + o.OrderDetails.Max(int (od) => od.ProductID)));
    ...
    public virtual Task Where_OrderBy_Count_client_eval(bool async)
        => AssertTranslationFailed(() => AssertCount(
            async,
    ...
    public virtual Task Single_Throws(bool async)
        => Assert.ThrowsAsync<InvalidOperationException>(async () => await AssertSingle(async, ss => ss.Set<Customer>()));
    ...
    public virtual Task First_inside_subquery_gets_client_evaluated(bool async)
        => AssertQuery(
            async,
            ss => ss.Set<Customer>().Where(c
                => c.CustomerID == "ALFKI" && c.Orders.Where(o => o.CustomerID == "ALFKI").First().CustomerID == "ALFKI"));
    ...
    public virtual async Task Contains_with_subquery_and_local_array_closure(bool async)
    {
        ...
        await AssertQuery(
            async,
            ss => ss.Set<Customer>()
                .Where(c => ss.Set<Customer>().Where(c1 => ids.Contains(c1.City)).Any(e => e.CustomerID == c.CustomerID)));
    ...
    public virtual Task Contains_with_local_list_inline(bool async)
        => AssertQuery(
            async,
            ss => ss.Set<Customer>().Where(c => new List<string> { "ABCDE", "ALFKI" }.Contains(c.CustomerID)));
    ...
    public virtual async Task Contains_with_local_enumerable_inline(bool async)
        => await AssertQuery(
            async,
            ss => ss.Set<Customer>().Where(c => new List<string> { "ABCDE", "ALFKI" }.Where(e => e != null).Contains(c.CustomerID)));
    ...
    public virtual Task Contains_with_local_ordered_enumerable_inline(bool async)
        => AssertQuery(
            async,
            ss => ss.Set<Customer>().Where(c => new List<string> { "ABCDE", "ALFKI" }.Order().Contains(c.CustomerID)));
    ...
    public virtual Task Contains_with_local_read_only_collection_inline(bool async)
        => AssertQuery(
            async,
            ss => ss.Set<Customer>().Where(c => new List<string> { "ABCDE", "ALFKI" }.AsReadOnly().Contains(c.CustomerID)));
    ...
    public virtual async Task Contains_with_local_non_primitive_list_inline_closure_mix(bool async)
    {
        ...
        await AssertQuery(
            async,
            ss => ss.Set<Customer>().Where(c => new List<Customer> { new() { CustomerID = "ABCDE" }, new() { CustomerID = id } }
    ...
    public virtual Task Contains_with_local_non_primitive_list_closure_mix(bool async)
    {
        ...
        return AssertQuery(
            async,
            ss => ss.Set<Customer>().Where(c => ids.Select(i => i.CustomerID).Contains(c.CustomerID)));
    }
    ...
    public virtual Task List_Contains_with_constant_list(bool async)
        => AssertQuery(
            async,
            ss => ss.Set<Customer>().Where(c
                => new List<Customer> { new() { CustomerID = "ALFKI" }, new() { CustomerID = "ANATR" } }.Contains(c)));
    ...
    public virtual Task Contains_with_constant_list_value_type_id(bool async)
        => AssertQuery(
            async,
            ss => ss.Set<Order>().Where(o => new List<Order> { new() { OrderID = 10248 }, new() { OrderID = 10249 } }.Contains(o)));
    ...
    private static readonly IEnumerable<string> _customers = ["ALFKI", "WRONG"];
    ...
    public virtual Task Contains_over_entityType_with_null_in_projection(bool async)
        => AssertQuery(
            async,
            ss => ss.Set<Order>().Where(o => ss.Set<Order>().Where(o => o.CustomerID == "VINET")
                .Contains(null)),
            assertEmpty: true);
    ...
    public virtual Task Contains_over_scalar_with_null_should_rewrite_to_identity_equality_subquery(bool async)
        => AssertQuery(
            async,
            ss => ss.Set<Order>().Where(o => ss.Set<Order>().Where(o => o.CustomerID == "VINET").Select(o => o.EmployeeID).Contains(null)),
            assertEmpty: true);
    ...
    public virtual Task Contains_over_entityType_with_null_should_rewrite_to_identity_equality_subquery_negated(bool async)
        => AssertQuery(
            async,
            ss => ss.Set<Order>()
                .Where(o => !ss.Set<Order>().Where(o => o.CustomerID == "VINET").Select(o => o.EmployeeID).Contains(null)));

    [ConditionalTheory, MemberData(nameof(IsAsyncData))]
    public virtual Task Contains_over_entityType_with_null_should_rewrite_to_identity_equality_subquery_complex(bool async)
        => AssertQuery(
            async,
            ss => ss.Set<Order>().Where(o => ss.Set<Order>().Where(o => o.CustomerID == "VINET").Select(o => o.EmployeeID)
                    .Contains(null)
    ...
    public virtual Task Contains_over_nullable_scalar_with_null_in_subquery_translated_correctly(bool async)
        => AssertQueryScalar(
            async,
            ss => ss.Set<Order>()
                .Select(o => ss.Set<Order>().Where(o => o.CustomerID == "VINET").Select(o => o.EmployeeID).Contains(null)));

    [ConditionalTheory, MemberData(nameof(IsAsyncData))]
    public virtual Task Contains_over_non_nullable_scalar_with_null_in_subquery_simplifies_to_false(bool async)
        => AssertQueryScalar(
            async,
            ss => ss.Set<Order>().Select(o
                => ss.Set<Customer>().Where(o => o.CustomerID != "VINET").Select(o => o.CustomerID).Contains(null)));
    ...
    public virtual Task Where_subquery_all_not_equals(bool async)
        => AssertQuery(
            async,
            ss => ss.Set<Customer>().Where(c => new List<string>
            {
                "ABCDE",
                "ALFKI",
                "ANATR"
    ...
    [ConditionalTheory, MemberData(nameof(IsAsyncData))] // #32374
